...
void AxePreset::setPresetNumber(int number) { 
	_preset = number; 
	status |= STATUS_PRESET_NUMBER_RECEIVED; 
}

void AxePreset::setSceneNumber(int number) { 
	_scene = number; 
	status |= STATUS_SCENE_NUMBER_RECEIVED; 
}

void AxePreset::setPresetName(char *buffer) {
	snprintf(_presetName, MAX_PRESET_NAME+1, buffer);
	status |= STATUS_PRESET_NAME_RECEIVED;
}

void AxePreset::setSceneName(char *buffer) {
	snprintf(_sceneName, MAX_SCENE_NAME+1, buffer);
	status |= STATUS_SCENE_NAME_RECEIVED;
}

void AxePreset::setEffects(const EffectId effectIdList[], const bool bypassedList[], const unsigned count) {
	for (_effectCount=0; _effectCount < (int) count && _effectCount < MAX_EFFECTS; _effectCount++) {
		_effectIdList[_effectCount] = effectIdList[_effectCount];
		_bypassedList[_effectCount] = bypassedList[_effectCount];
	}
	status |= STATUS_EFFECT_LIST_RECEIVED;
}
...
void AxePreset::copyEffectNameAndTag(EffectId effectId, char *name, byte szName, char *tag,	byte szTag) {
  switch (effectId) {
...
bool AxePreset::isSwitchable(EffectId effectId) {
  switch (effectId) {
		case ID_CONTROL:
		case ID_CAB1:
		case ID_CAB2:
